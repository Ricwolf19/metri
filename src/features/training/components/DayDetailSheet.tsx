...
const statusDot = (status: string, brand: string): string =>
  status === 'trained' ? brand : status === 'rest' ? '#52525b' : 'rgba(239,68,68,0.85)';
...
export const DayDetailSheet = ({
  ...
  return (
    <Sheet visible={date !== null} onClose={onClose} className="max-h-[88%]">
      <Text className="text-lg font-sans-bold text-ink-50">{title}</Text>

      {adherence ? (
        <View className="mt-2 flex-row items-center gap-2.5">
          <View
            style={{ backgroundColor: statusDot(adherence.status, brand) }}
      ...
      <ScrollArea maxHeight={520} className="mt-2">
        {workouts.map((w) => (
